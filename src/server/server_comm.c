...
// ============================================================================
// Key Validation (for QR claiming — does not require init)
// ============================================================================
...
int server_comm_validate_key(const char *url, const char *api_key) {
    ...
    if (http_parse_url(url, host, sizeof(host), &port, path, sizeof(path),
                       HEARTBEAT_PATH) < 0) {
        LOG_ERROR("validate_key: invalid server URL: %s", url);
        return -1;
    }
    ...
    if (strncmp(url, "https://", 8) == 0) {
        if (tls_available()) {
            ...
            LOG_ERROR("validate_key: HTTPS required but TLS not available");
            return -1;
        }
    }

    // Build minimal heartbeat body
    cJSON *req_json = cJSON_CreateObject();
    cJSON_AddStringToObject(req_json, "unit_id", "pending-claim");
    cJSON_AddBoolToObject(req_json, "armed", false);
    cJSON_AddStringToObject(req_json, "firmware_version", FIRMWARE_VERSION);
    cJSON_AddNumberToObject(req_json, "uptime_seconds", 0);
    cJSON_AddNumberToObject(req_json, "free_storage_mb", 0);
    cJSON_AddNumberToObject(req_json, "pending_clips", 0);

    char *body = cJSON_PrintUnformatted(req_json);
    cJSON_Delete(req_json);

    if (!body) {
        LOG_ERROR("validate_key: failed to build request body");
        return -1;
    }

    // Copy API key to a local buffer that we can securely clear
    char key_copy[64];
    strncpy(key_copy, api_key, sizeof(key_copy) - 1);
