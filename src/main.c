...
void apis_preallocate_frame(void) {
    if (!g_frame_prealloc) {
        g_frame_prealloc = malloc(sizeof(frame_t));
    }
}
...
static int run_capture_loop(const config_t *config) {
    ...
    if (!frame) {
        frame = malloc(sizeof(frame_t));  // Fallback for non-ESP32
    }
    if (!frame) {
    ...
    while (g_running) {
        ...
        consecutive_failures = 0;
        LOG_INFO(">>> frame OK seq=%u", frame->sequence);

        // QR claiming: scan when device is UNCLAIMED (has WiFi but no API key).
        // On ESP32, raw grayscale was fed into quirc during camera_read()
        // (307KB copy while holding fb). Here we just run quirc detection.
        // On other platforms, use the BGR→gray scan_frame() path.
        if (led_controller_is_state_active(LED_STATE_UNCLAIMED) &&
            qr_scanner_is_initialized()) {
            qr_scan_result_t qr = {0};
#ifdef APIS_PLATFORM_ESP32
            LOG_INFO(">>> QR process start");
            qr_scanner_status_t qr_status = qr_scanner_process(&qr);
            LOG_INFO(">>> QR process done, status=%d found=%d", qr_status, qr.found);
#else
...
#ifdef APIS_PLATFORM_ESP32
                    config_manager_set_server(qr.server_url, qr.api_key);
                    led_controller_clear_state(LED_STATE_UNCLAIMED);
                    led_controller_set_state(LED_STATE_DISARMED);
                    LOG_INFO("Device claimed via QR — server: %s", qr.server_url);
                    server_comm_init();
                    server_comm_start();
#else
        ...
        // only needs QR scanning. motion_detect() touches ~3.3MB across 6
        // PSRAM arrays which causes cache thrashing on ESP32-S3, stalling
        // the CPU long enough to trigger the task watchdog.
        if (led_controller_is_state_active(LED_STATE_UNCLAIMED)) {
