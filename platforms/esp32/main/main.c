...
void app_main(void) {
    ...
    // Step 3: Pre-allocate large PSRAM buffers BEFORE camera DMA starts.
    ...
    if (qr_scanner_init() == QR_SCANNER_OK) {
        ESP_LOGI(TAG, "QR scanner pre-allocated (~500KB PSRAM)");
    } else {
        ESP_LOGW(TAG, "QR scanner pre-allocation failed");
    }
    if (motion_init(NULL) == MOTION_OK) {
        ESP_LOGI(TAG, "Motion detection pre-allocated (~2MB PSRAM)");
    } else {
        ESP_LOGW(TAG, "Motion detection pre-allocation failed");
    }
    // Rolling buffer: 3 frames × 921KB each ≈ 2.7MB PSRAM.
    // Must allocate before camera DMA starts (MSPI bus contention
    // during large PSRAM calloc causes INT WDT timeout).
    ...
    if (rolling_buffer_init(&buf_cfg) == ROLLING_BUFFER_OK) {
        ESP_LOGI(TAG, "Rolling buffer pre-allocated (~2.7MB PSRAM)");
    } else {
        ESP_LOGW(TAG, "Rolling buffer pre-allocation failed");
    }
    // Frame buffer used by capture loop (~921KB PSRAM).
    apis_preallocate_frame();
    ESP_LOGI(TAG, "Frame buffer pre-allocated (~900KB PSRAM)");

    // Step 4: Initialize WiFi BEFORE camera.
    ...
    // With INT WDT disabled, the PSRAM allocation slowdown during camera_init
    // (after WiFi) won't trigger a watchdog — just takes a moment longer.
    ...
    // Step 5: Initialize camera AFTER WiFi.
    // Camera DMA and WiFi both use the MSPI bus for PSRAM. Starting camera
    // after WiFi is stable avoids EV-EOF-OVF (DMA event overflow) that
    // corrupts the cam_task state. The fb_get() call in the capture loop
    // will get the first valid frame immediately.
    ESP_LOGI(TAG, "Initializing camera...");
    if (camera_init(NULL) == CAMERA_OK) {
        ESP_LOGI(TAG, "Camera initialized");
    } else {
        ESP_LOGW(TAG, "Camera init failed — will retry in capture loop");
    }

    // Step 6: Launch platform-agnostic APIS main.
    // camera_init(), qr_scanner_init(), and motion_init() already called
    // above — the calls in run_capture_loop() will be no-ops.
