...
camera_status_t camera_init(const apis_camera_config_t *config) {
    ...
    camera_config_t esp_cam_config = {
        ...
        // Grayscale: 307KB per frame, converted to BGR with a sequential
        // loop that the PSRAM cache handles well (~10-30ms).
        .pixel_format = PIXFORMAT_GRAYSCALE,
        .frame_size = FRAMESIZE_VGA,       // 640x480 — best resolution for QR scanning
        .jpeg_quality = 10,                // Ignored for grayscale
        .fb_count = 2,                     // 2 grayscale buffers (~614KB in PSRAM)
        .fb_location = CAMERA_FB_IN_PSRAM,
        .grab_mode = CAMERA_GRAB_LATEST,   // Always get freshest frame
    };
...
camera_status_t camera_read(frame_t *frame, uint32_t timeout_ms) {
    ...
    if (fb->format == PIXFORMAT_GRAYSCALE) {
        ...
        if (qr_scanner_is_initialized()) {
            LOG_INFO(">>> feed_gray start");
            qr_scanner_feed_grayscale(fb->buf, FRAME_WIDTH, FRAME_HEIGHT);
            LOG_INFO(">>> feed_gray done");
        }
        ...
        // Do NOT read fb->width/height after conversion — the
        // grayscale_to_bgr24 writes 921KB to PSRAM which triggers MSPI
        // bus contention with camera DMA. This causes PSRAM cache
        // corruption: both the frame metadata (stored after data[]) and
        // even the camera_fb_t fields read back as garbage. Since the
        // camera is always configured for VGA, use constants instead.
        frame->width = FRAME_WIDTH;
        frame->height = FRAME_HEIGHT;
        esp_camera_fb_return(fb);
        fb = NULL;
    } else if (fb->format == PIXFORMAT_JPEG) {
        // Fallback JPEG path — only used if camera is reconfigured at runtime
        if (!fmt2rgb888(fb->buf, fb->len, fb->format, frame->data)) {
            LOG_ERROR("JPEG decode failed (len=%zu)", (size_t)fb->len);
            esp_camera_fb_return(fb);
            g_frames_dropped++;
            return CAMERA_ERROR_READ_FAILED;
        }
        // Swap RGB → BGR in-place
        for (size_t i = 0; i < pixel_count; i++) {
            uint8_t tmp = frame->data[i * 3];
            frame->data[i * 3] = frame->data[i * 3 + 2];
            frame->data[i * 3 + 2] = tmp;
        }
...
void camera_drain_start(void) {
    ...
    xTaskCreatePinnedToCore(camera_drain_task, "cam_drain", 4096, NULL,
                            5, &g_drain_task_handle, 1);
